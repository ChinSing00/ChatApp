package com.example.vogabond.chatapp.session;
...
import com.example.vogabond.chatapp.session.search.SearchMessageActivity;
import com.example.vogabond.chatapp.session.viewholder.MsgViewHolderAVChat;
import com.example.vogabond.chatapp.session.viewholder.MsgViewHolderFile;
        ...
        if (p2pCustomization == null) {
            ...
            SessionCustomization.OptionsButton infoButton = new SessionCustomization.OptionsButton() {
                ...
                public void onClick(Context context, View view, String sessionId) {
                   ...
                   MSG_InfoActivity.startActivity(context, sessionId); //打开聊天信息
                }
            };
            ...
            teamCustomization = new SessionCustomization() {
                ...
                public void onActivityResult(Activity activity, int requestCode, int resultCode, Intent data) {
                    if (requestCode == TeamRequestCode.REQUEST_CODE) {
                        ...
                        if (requestCode == TeamRequestCode.REQUEST_TEAM_VIDEO) {
                            if (resultCode == Activity.RESULT_OK) {
        ...
        public void onItemClick(final PopupMenuItem item) {
            switch (item.getTag()) {
                case ACTION_HISTORY_QUERY:
                    MSG_HistoryAcitivity.start(item.getContext(), item.getSessionId(), item.getSessionTypeEnum()); // 漫游消息查询
                    break;
