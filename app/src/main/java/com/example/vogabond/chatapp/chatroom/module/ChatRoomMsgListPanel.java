package com.example.vogabond.chatapp.chatroom.module;
...
public class ChatRoomMsgListPanel {
    ...
    private void init() {
        initListView();
        this.uiHandler = new Handler(MyCache.getContext().getMainLooper());
    ...
    private void initListView() {
        ...
        adapter.setOnFetchMoreListener(new MessageLoader());
        messageListView.setAdapter(adapter);
    }
    public void onIncomingMessage(List<ChatRoomMessage> messages){
        boolean needScrollToButton = isLastMessageVisible();
        ...
        if (isMyMessage(lastMsg) && needScrollToButton){
            doScrollToBottom();
        }
    }
    ...
    //发送消息后，更新本地消息列表
    private void onMsgSend(ChatRoomMessage message){
        saveMessage(message);
    ...
    private void registerObservers(boolean register) {
        ...
        service.observeAttachmentProgress(attachmenProgressObserver, register);
    }
    //消息状态变化观察者
    ...
    //消息附件上传、下载进度观察者
    Observer<AttachmentProgress> attachmenProgressObserver = new Observer<AttachmentProgress>() {
        @Override
    ...
    private class MsgItemEventListener implements ChatRoomMsgAdapter.ViewHolderEventListener{
        ...
        private void onFailedBtonClicl(IMMessage message){
            if (message.getDirect() == MsgDirectionEnum.Out){
                //发出的消息，如果是发送失败的，直接重发，否则有可能是漫游到多媒体消息，
        ...
        public boolean onViewHodlerLongClick(View clickView, View viewHolderView, IMMessage item){
            return true;
        }
        ...
        @Override
        public void onFailedBtonClick(IMMessage resendMessage) {
