package com.example.vogabond.chatapp.chatroom.activity;
...
import com.example.vogabond.chatapp.chatroom.fragment.ChatRoomMessageFragment;
...
public class ChatRoomActivity extends UI {
    ...
    private boolean hasEnterSuccess = false;//是否已经成功登陆聊天室
    ...
    private void enterRoom() {
        ...
        enterRequest.setCallback(new RequestCallback<EnterChatRoomResultData>() {
            ...
            public void onFailed(int code) {
                LogUtil.ui("enter char room failed,callback code ="+code);
                onLoginDone();
                if (code == ResponseCode.RES_CHATROOM_BLACKLIST){
                    Toast.makeText(ChatRoomActivity.this,"你已经被拉入黑名单，不能再进入",Toast.LENGTH_SHORT).show();
    ...
    Observer<ChatRoomStatusChangeData> onlineStatus = new Observer<ChatRoomStatusChangeData>() {
        ...
        public void onEvent(ChatRoomStatusChangeData chatRoomStatusChangeData) {
            ...
            if (chatRoomStatusChangeData.status == StatusCode.CONNECTING){
                ...
                //登录成功后，断网重连交给云信的SDK，如果重连失败，可查询具体失败原因
                if (hasEnterSuccess){
