package com.example.vogabond.chatapp.chatroom.thridparty;
...
import com.example.vogabond.chatapp.chatroom.helper.ExtensionHelper;
...
public class ChatRoomHttpClient {
   private static final  String TAG = ChatRoomHttpClient.class.getSimpleName();
    ...
    private static final String RESULT_KEY_ROOM_ID = "rooid";
     ...
     */
    public void fatchChatRoomList(final ChatRoomHttpCallback<List<ChatRoomInfo>> callback){
        ...
        NimHttpClient.getInstance().execute(url, headers, null, false, new NimHttpClient.NimHttpCallback() {
            ...
            public void onResponse(String response, int code, Throwable exception) {
                if (code != 200 || exception != null){
                    LogUtil.e(TAG,"fetcjChatRoomList failed : code =" + code + ", errorMsg ="
    ...
    private String readAppkey() {
        try {
            ApplicationInfo appInfo = MyCache.getContext().getPackageManager()
                    .getApplicationInfo(MyCache.getContext().getPackageName(), PackageManager.GET_META_DATA);
            if (appInfo != null){
